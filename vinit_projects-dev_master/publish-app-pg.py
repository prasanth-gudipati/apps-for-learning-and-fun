...
class RestApi(object):
    def __init__(self,ecp_ip,user,password):
        ...
        #    payload = ''
        #    url = "https://%s" % self.ecp_ip

        #    response = requests.request("GET", url, headers=self.headers, data=payload, verify=False, timeout=60)
        #    tmp_token = response.cookies['ECP-CSRF-TOKEN']

        #    headers = {'Content-Type': 'application/json','Accept': 'application/json'}
        #    headers['X-CSRF-TOKEN'] = tmp_token

        #    self.logger.banner_log(thread, 'login to ecp and fetch session cookie')
        ...
        #        self.logger.error_log(thread,'Api Response Status code: %s' % session.status_code)
        #        self.logger.error_log(thread,'Api Response Reason: %s' % session.reason)
        #        self.logger.error_log(thread,'Api Response Text: %s' % json.dumps(json.loads(session.text),indent=4))
        #        return sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tenant',
                        help='tenant name', default = 'provider-org')
    ##parser.add_argument('-csv',
    ##                    help='csv file path', default='dummy.csv')
